...
class MousePositionTracker(tk.Frame):
    ...
    def quit(self, event):
        ...
        im1 = img.crop((*self.start, *self.end))
        im1.save(self.path.replace("weapon_classification", "small").split(".")[0] + f"{self.start[0]}_{self.start[1]}_{self.end[0]}_{self.end[1]}.png")
...
class Application(tk.Frame):
    ...
    def __init__(self, parent, path, *args, **kwargs):
        ...
        self.posn_tracker = MousePositionTracker(self.canvas, path)
        self.posn_tracker.autodraw(command=on_drag)  # Enable callbacks.


if __name__ == '__main__':

    for dir, _, files in os.walk("weapon_classification\\Rapid Light Missile Launcher II", topdown=False):
        for file in files:

            os.makedirs(dir.replace("weapon_classification", "small"), exist_ok=True)
            ...
            app = Application(root, os.path.join(dir, file), background=BACKGROUND)
